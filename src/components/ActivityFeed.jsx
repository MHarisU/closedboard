...
import { Plus, CheckCircle, ArrowRightLeft, Pencil, Trash2, Pin, Activity } from 'lucide-react';
  ...
  return (
    <div className={`rounded-2xl border p-4 transition-colors duration-300
      ...
      <h3 className={`font-semibold mb-4 flex items-center gap-2
        ...
        <Activity size={16} className={isDark ? 'text-slate-400' : 'text-slate-500'} />
        Activity
      ...
      <div className="space-y-2.5 max-h-[280px] overflow-y-auto column-scroll">
        {history.length === 0 ? (
          ...
          history.slice(0, 15).map((entry, index) => {
            const ActionIcon = actionIcons[entry.action] || Pin;
            return (
              <div key={entry.id}
              ...
              >
                <span className={`shrink-0 mt-0.5 ${getActionColor(entry.action)}`}>
                  <ActionIcon size={15} />
                </span>
