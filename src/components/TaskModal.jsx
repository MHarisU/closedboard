...
import { Pencil, GraduationCap, Plus, BookOpen, Square, X, BookMarked, ExternalLink, Bot } from 'lucide-react';
import { PRIORITIES, TAGS, COLUMNS, LEARNING_TOPICS } from '../utils/constants';
...
export default function TaskModal({ isOpen, onClose, onSave, editTask }) {
  ...
  const addResource = () => {
    ...
    setResources(prev => [...prev, { title: newResourceTitle.trim(), url: newResourceUrl.trim() }]);
      ...
      <div className={`w-full max-w-lg rounded-2xl shadow-2xl animate-slide-in max-h-[90vh] overflow-hidden
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4 overflow-y-auto max-h-[calc(90vh-140px)]">
              ...
              {subtasks.map((subtask, index) => (
                <div key={index} className={`flex items-center gap-2 text-sm p-2 rounded-lg
                  ...
                  <span className={isDark ? 'text-slate-500' : 'text-slate-400'}>
                    {isLearningTask ? <BookOpen size={14} /> : <Square size={14} />}
                  </span>
          ...
          {isLearningTask && (
            <div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <input type="text" value={newResourceTitle}
                    onChange={(e) => setNewResourceTitle(e.target.value)} placeholder="Title"
                    className={`flex-1 px-3 py-2 rounded-lg text-sm
                      ${isDark
                        ? 'bg-slate-800 border-slate-700 text-white placeholder-slate-500'
                        : 'bg-slate-50 border-slate-200 text-slate-900 placeholder-slate-400'}
                      border focus:ring-2 focus:ring-blue-500/20 focus:outline-none`}
                  />
                  <input type="url" value={newResourceUrl}
                    onChange={(e) => setNewResourceUrl(e.target.value)} placeholder="https://..."
                    className={`flex-1 px-3 py-2 rounded-lg text-sm
                      ${isDark
                        ? 'bg-slate-800 border-slate-700 text-white placeholder-slate-500'
                        : 'bg-slate-50 border-slate-200 text-slate-900 placeholder-slate-400'}
          ...
          <label className={`flex items-center gap-2.5 cursor-pointer p-2.5 -mx-2.5 rounded-lg transition-colors
            ...
            <input type="checkbox" checked={isAITask}
              onChange={(e) => setIsAITask(e.target.checked)} className="w-4 h-4 rounded"
            />
