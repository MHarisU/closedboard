...
import { TAGS, LEARNING_TOPICS, isLearningTask } from '../utils/constants';
    ...
    <div className={`mb-6 rounded-2xl border overflow-hidden transition-all duration-300
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <ChevronDown size={16}
            className={`transition-transform ${isDark ? 'text-slate-400' : 'text-slate-500'} ${expanded ? 'rotate-180' : ''}`} />
        </div>
      </button>
            ...
            {topicStats.map(topic => (
              ...
              >
                ...
                <div className={`h-2 rounded-full overflow-hidden mb-3 ${isDark ? 'bg-slate-700' : 'bg-slate-200'}`}>
                  <div className={`h-full transition-all duration-500 ${topic.tag.color}`}
