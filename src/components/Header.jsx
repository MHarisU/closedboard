...
export default function Header({
  ...
  useEffect(() => {
    const updateSyncText = () => {
      if (!lastSync) { setSyncText(''); return; }
      const diff = Date.now() - lastSync;
      if (diff < 5000) setSyncText('Just now');
      else if (diff < 60000) setSyncText(`${Math.floor(diff / 1000)}s ago`);
      else setSyncText(`${Math.floor(diff / 60000)}m ago`);
    };
    updateSyncText();
